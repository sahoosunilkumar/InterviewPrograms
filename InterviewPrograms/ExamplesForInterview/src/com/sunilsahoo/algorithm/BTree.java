 ...
 *  
 *  
 *  The songs themselves are write once, read rest of the song's lifetime. So, 
 *  I would not change the way the song is stored...a sequentially written file. 
 *  The directory that maintains the list of songs could and will be randomly accessed. 
 *  So, this directory data structure needs to support random directory reads. 
 *  In addition, it seems that a consecutive play should not repeat a song 
 *  i.e. this calls for sequential directory reads. 
 *  One good data structure that supports both efficient random reads (log n) and 
 *  supports sequential reads is a btree. 
 *  Other structures like a skiplist provides both random and sequential access 
 *  with its own tradeoffs.
 *  
 *  In computer science, a B-tree is a self-balancing tree data structure that 
 *  keeps data sorted and allows searches, sequential access, insertions, 
 *  and deletions in logarithmic time. The B-tree is a generalization of a binary search tree 
 ...
 *  that read and write large blocks of data. B-trees are a good example of a data structure 
