...
public class QuickSort {
	 ...
	 * 
	 * 
	 * there are three basic steps: 1. Partition the array or subarray into left
	 * (smaller keys) and right (larger keys) groups. 2. Call ourselves to sort
	 * the left group. 3. Call ourselves again to sort the right group.
	 * 
	 ...
	 * subarrays? By calling ourself recursively. If the array has two or more
	 * cells, the algorithm calls the partitionIt() method, described in the
	 * preceding section, to partition it. This method returns the index number
	 * of the partition: the left element in the right (larger keys) subarray.
	 * The partition marks the boundary between the subarrays.
	 * 
	 * What pivot value should the partitionIt() method use? Here are some
	 * relevant ideas: • The pivot value should be the key value of an actual
	 * data item; this item is called the pivot. • You can pick a data item to
	 * be the pivot more or less at random. For simplicity, let’s say we always
	 * pick the item on the right end of the subarray being parti- tioned. •
	 * After the partition, if the pivot is inserted at the boundary between the
	 * left and right subarrays, it will be in its final sorted position
	 * 
	...
	private static int partitionIt(int left, int right, long pivot,
		...
		while (true) {
			...
			if (leftPtr >= rightPtr)
				...
				System.out.println("leftPtr : " + leftPtr + " rightPtr : "
						+ rightPtr + " pivot : " + pivot);
				exchange(leftPtr, rightPtr, numbers); // if pointers cross,
														// partition done // not
														// crossed, so swap
														// elements
			}
