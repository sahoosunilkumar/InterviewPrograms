...
/**
 * Straight to the Matter Please, do not expect any complex mathematical
 * discoveries. The thing is that in reality Timsort is not a standalone
 * algorithm but a hybrid, an efficient combination of a number of other
 * algorithms, seasoned with the author’s ideas. The mechanism of the algorithm
 * can be briefly explained as follows: 1.
 * Aparticularalgorithmisusedtosplittheinputarrayintosub-arrays. 2.
 * Eachsub-arrayissortedwithasimpleInsertionSort. 3.
 * Thesortedsub-arraysaremergedintoonearraywiththeuseofMergeSort.Asusual,
 * thedevilhidesinthe details, namely, in the algorithm in p. 1 and the merge
 * sort modification in p. 3. The Algorithm Definitions N: the length of the
 * input array run: an ordered sub-array in the input array. At the same time,
 * the order is non-descending or strictly descending, i.e. “a0 ≤ a1 ≤ a2 ≤ ...»
 * or «a0 > a1 > a2 > ...” minrun: as it was said above, in the first step of
 * the algorithm, the input array is split into runs. minrun is a minimum length
 * of such run. This number is calculated by a certain logics from the N number.
 * 
 * 
 * 
 * Step 0. Calculation of the Minrun. The minrun number is determined based on
 * the N, by the following principles: 1.
 * Itshouldn’tbetoolongastheminrunwilllaterbesubjectedtoinsertionsort,
 * whichiseffectiveforshort runs. 2. Itshouldn’tbetooshort,as,theshortertherun,
 * themorerunswillhavetobemergedinthenextstep. 3. It would be good if N \ minrun
 * was a power of 2 (or close to it). This requirement results from the fact
 * that the merge sort performs best on the runs of about the same length.
 * 
 * 
 * Step 1. Splitting into Runs and Their Sorting. So, at this stage, there is an
 * input array with N size and a calculated minrun. The algorithm for this step:
 * 1. Thebaseaddressofthecurrentelementissetatthebeginningoftheinputarray. 2.
 * Startingwiththecurrentarray,searchfortherun(anorderedarray)intheinputarray.
 * Bydefinition,thisrun will definitely include the current element and the one
 * that follows, but further on it’s pure luck. If the final array is
 * descending, the elements are ordered in a non-descending order (it’s a simple
 * serial algorithm, it’s just that we move from both sides to the centre,
 * changing the places of the elements). 3.
 * Ifthecurrentrunissmallerthantheminrun,
 * takethenumberofelementsthatfollowthefoundrunequalto the minrun — size(run).
 * Thus, the final run will be the size of the minrun or more, a part of which
 * (ideally all of it) is ordered. 4.
 * Thenthisrunisinsertionsorted.Asthisrunissmallandpartiallyordered,
 * thesortingisfastandhighly performing. 5.
 * Thebaseaddressofthecurrentelementissetattheelementfollowingthisrun. 6.
 * Iftheendoftheinputarrayhasnotbeenreached,gotopoint2,
 * otherwiseitistheendofthisstep.
 * 
 * 
 * Step 2. Merge At this stage, there is an input array split into runs. If the
 * data in the input array were close to random, the run size is close to
 * minrun, and if the data had ordered ranges (the algorithm application
 * requirement lets us hope for it), the run size exceeds minrun. Now, these
 * runs need to be combined to receive the final and completely ordered array,
 * and in the course of it, two requirements have to be satisfied: 1.
 * Thereshouldbecombinedrunsofrelativelyequalsize(thiswayitwillbemoreefficient).
 * 2. Thealgorithmstabilityshouldbemaintained,i.e.nouselessshifts(e.g.
 * twoconsecutiveequalnumbers should not exchange places). This is achieved in
 * the following way: 1. Create an empty pair stack <run base address>–<run
 * size>. Take the first run. 2. Addafewdatatothestack<baseaddress>–
 * <size>tothecurrentrun. 3.
 * Assesswhetherthecurrentrunshouldbemergedwiththeprecedingone.Todothis,
 * checkwherethetwo requirements are satisfied (say, X, Y and Z are three top
 * runs in the stack): X > Y + Z Y > Z 4. Ifoneoftherequirementsisnotsatisfied,
 * arrayYismergedwiththesmallestofarraysXandZ.Thisstepis performed until both
 * requirements are satisfied or until all data is ordered. 5.
 * Foranyunconsideredruns,takethenextandgopoint2.Otherwise,itistheend.
 * 
 * @author sunilkumarsahoo
 *
 */
public class TimSortGoogle {
...
class StijnSort<T> {
	 ...
	 */
	private StijnSort(T[] a, Comparator<? super T> c) {
		this.a = a;
		this.c = c;
	...
	static <T> void sort(T[] a, int lo, int hi, Comparator<? super T> c) {
		 ...
		 */
		StijnSort<T> ts = new StijnSort<T>(a, c);
	...
	private static int minRunLength(int n) {
		...
		// System.out.println("minRunlength(" + n + ")=" + (n+r));
		return n + r;
	}
	...
	private void newMergeCollapse() {
		while (stackSize > 1) {
			int n = stackSize - 2;
			if (n > 0 && runLen[n - 1] <= runLen[n] + runLen[n + 1] || n - 1 > 0
	...
	private void mergeHi(int base1, int len1, int base2, int len2) {
		...
		outer: while (true) {
			...
			do {
				assert len1 > 0 && len2 > 1;
				count1 = len1 - gallopRight(tmp[cursor2], a, base1, len1,
						len1 - 1, c);
				if (count1 != 0) {
					dest -= count1;
					cursor1 -= count1;
