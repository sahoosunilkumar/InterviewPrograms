...
public class FileCopy {
	...
	public static void main(String[] args) {
		...
		fileCopy.copy(sourcePath, destinationPath, "00:00:38", "00:03:42",
				"00:03:42");
		// fileCopy.copy(sourcePath, destinationPath, (int)new
		// File(sourcePath).length()-1024, (int)new File(sourcePath).length());
	}

	// private void copy(String sourcePath, String destinationPath, int
	// startIndex, int endIndex){
	// System.out.println("inside copy startIndex : "+startIndex+" endIndex :
	// "+endIndex);
	...
	// buffer = ByteBuffer.wrap(new byte[DEFAULT_BUFFER_SIZE<availableLength ?
	// DEFAULT_BUFFER_SIZE : availableLength]);
	...
	private void copy(String sourcePath, String destinationPath, int startIndex,
			int endIndex) {
		System.out.println("inside copy startIndex : " + startIndex
				+ " endIndex : " + endIndex);
		...
		while (startIndex < endIndex) {
			...
			try {
				...
				int availableLength = (endIndex - startIndex);
				buffer = ByteBuffer
						.wrap(new byte[DEFAULT_BUFFER_SIZE < availableLength
								? DEFAULT_BUFFER_SIZE : availableLength]);
				// System.out.println("startIndex : "+startIndex+" endIndex :
				// "+endIndex+" length : "+buffer.array().length);
				FileChannel fc = fis.getChannel();
				fc.position(startIndex);
				int readCount = fc.read(buffer);
				if (readCount == -1) {
	...
	private void copy(String sourcePath, String destinationPath,
			String startDuration, String endDuration, String totalDuration) {
	...
	private int getDurationInSeconds(String duration) {
		...
		return Integer.valueOf(durationArr[0]) * 3600
				+ Integer.valueOf(durationArr[1]) * 60
				+ Integer.valueOf(durationArr[2]);
