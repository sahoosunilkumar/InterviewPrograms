...
//		System.out.println(matches+" "+str.matches(regex));
		...
		System.out.println(practise.checkBalanced("{[]}"));
//		String[] str = {"{","[","]","}"};
		try {
...
//		
        ...
        System.out.print(" "+arr1[leftIndexOfResult] +" : "+arr2[rightIndexOfResult]);
	}
	...
	static String checkBalanced(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			if ('(' == str.charAt(i) || '{' == str.charAt(i) || '[' == str.charAt(i)) {
				...
				if(isMatching(str.charAt(i), parenthesisStack.isEmpty()? ' ' :parenthesisStack.peek())){
					if ((')' == str.charAt(i) || '}' == str.charAt(i) || ']' == str.charAt(i))){
