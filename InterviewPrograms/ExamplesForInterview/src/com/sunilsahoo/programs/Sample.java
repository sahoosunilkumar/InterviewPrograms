...
public class Sample {
	...
	public static void main(String args[]) {
		...
		System.out.println("no of stops : " + sample.solution(
				new int[] { 60, 80, 40 }, new int[] { 2, 3, 5 }, 5, 2, 200));
		int[] data = { 5, 4, -3, 2, 0, 1, -1, 0, 2, -3, 4, -5 };
		sample.maxNoOfMatches(data);
		int[] arr = { 1, 5, 3, 3, 2, 0, 7 };
		// sample.solution(arr);
		System.out.println(
				"no of swaps required :" + sample.noOfSwapsRequired(arr));
		// System.out.println("no of swaps required
		// :"+sample.swapRequired(arr));
	...
	int solution(int A, int B) {
		...
		return perfectSquareUpperBounds - perfectSquareLowerBounds + 1;
	}
	...
	public boolean noOfSwapsRequired(int[] arr) {
		...
		System.out.println(Utility.toString(arr) + "sorted arr :"
				+ Utility.toString(sortedArr));
		int misMatch = 0;
		for (int i = 0; i < arr.length; i++) {
	...
	public int solution(int N, String S) {
		...
		String seatNumber = "";
		for (int i = 1; i <= N; i++) {
			for (int seatNumberCounter = 0; seatNumberCounter < seatNumberStr
					.length(); seatNumberCounter++) {

				seatNumber = String
						.valueOf(seatNumberStr.charAt(seatNumberCounter));
				if (S.contains(i + seatNumber)
						|| (aisleStartArr.contains(seatNumber)
								&& consecutiveCounter != 0)) {

					// System.out.println("failed "+(i+seatNumber)+" :
					// "+consecutiveCounter);
					consecutiveCounter = (aisleStartArr.contains(seatNumber)
							&& consecutiveCounter != 0) ? 1 : 0;
				} else {
					consecutiveCounter++;
					// System.out.println("adv "+(i)+seatNumber+" :
					// "+consecutiveCounter);
				}
